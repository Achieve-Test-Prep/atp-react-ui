...
> = function NavigationButton({ onClick, disabled, directionIcon }) {
  ...
  const icon =
    directionIcon === 'prev' ? ArrowSmallLeftIcon : ArrowSmallRightIcon;
...
export const PageButton: React.FC<React.PropsWithChildren<PageButtonProps>> =
  function PageButton({ page, isActive, onClick }) {
    return (
      <Button
        size="pagination"
        as={isActive ? 'contained' : 'link'}
        onClick={onClick}
      >
        {page}
      </Button>
...
export const Pagination = React.forwardRef<Ref, PaginationProps>(
  (props, ref) => {
    const {
      totalResults,
      resultsPerPage = 10,
